...
const uData = computed<number[][]>(() =>
  showThreshold.value
    ? [xData.value, avgSeries.value, threshSeries.value]
    : [xData.value, avgSeries.value],
)
...
const percentiles = computed(() => {
  ...
  const finite = (N > 0 ? src.slice(src.length - N) : []).filter(
...
const prefHist = computed({
  get: () => getPreferredExport(PREF_HIST) ?? lastHistFmt.value ?? 'png',
  set: (v: 'png' | 'csv') => setPreferredExport(PREF_HIST, v),
})
const prefLine = computed({
  get: () => getPreferredExport(PREF_LINE) ?? lastLineFmt.value ?? 'png',
  set: (v: 'png' | 'csv') => setPreferredExport(PREF_LINE, v),
})
    ...
    <div class="grid grid-cols-2 gap-2 h-full">
        ...
        <div class="flex items-center justify-between px-2 pt-1">
          ...
          <div class="flex gap-1 items-center">
            ...
            <button
              ...
              @click="exportHistUsingPreferred"
              aria-describedby="move-hist-desc"
              aria-label="Export using preferred format"
            >
              Export
            </button>
            <span v-if="lastHistFmt" class="badge badge-ghost badge-xs" aria-live="polite"
              >Last: {{ lastHistFmt?.toUpperCase?.() }}</span
            >
          </div>
        </div>
      ...
      <div class="bg-base-200/30 rounded-md rounded-md overflow-hidden">
        <div class="flex items-center justify-between px-2 pt-1">
          ...
          <div class="flex gap-1 items-center">
            <label
              class="label cursor-pointer gap-1 text-[10px]"
              :title="`Threshold ${threshold.toFixed?.(3) ?? threshold}`"
            >
              <input
                type="checkbox"
                class="toggle toggle-xs"
                v-model="showThreshold"
                aria-label="Toggle threshold line"
              />
              <span>Threshold</span>
            </label>
            <div class="hidden md:flex gap-1">
              <span class="badge badge-outline" :title="'P50 recent avg speed'"
                >P50: {{ percentiles.p50.toFixed(3) }}</span
              >
              <span class="badge badge-outline" :title="'P90 recent avg speed'"
                >P90: {{ percentiles.p90.toFixed(3) }}</span
              >
              <span class="badge badge-outline" :title="'P99 recent avg speed'"
                >P99: {{ percentiles.p99.toFixed(3) }}</span
              >
            </div>
            ...
            <button
              class="btn btn-primary btn-xs"
              @click="exportLineUsingPreferred"
              aria-describedby="move-line-desc"
              aria-label="Export using preferred format"
            >
              Export
            </button>
            <span v-if="lastLineFmt" class="badge badge-ghost badge-xs" aria-live="polite"
              >Last: {{ lastLineFmt?.toUpperCase?.() }}</span
            >
          </div>
        </div>
        <UPlotLine
          ...
          :data="uData"
          :options="uOpts"
          class="w-full h-[calc(100%-1.25rem)]"
        />
      </div>
    </div>
  </div>
