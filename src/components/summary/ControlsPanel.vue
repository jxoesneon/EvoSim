...
const mutationRate = computed<number>({ get: () => (store.simulationParams as any).mutationRate, set: v => store.setMutationRate?.(Number(v)) })
const mutationAmount = computed<number>({ get: () => (store.simulationParams as any).mutationAmount, set: v => store.setMutationAmount?.(Number(v)) })
const movementThreshold = computed<number>({ get: () => (store.simulationParams as any).movementThreshold, set: v => store.setMovementThreshold?.(Number(v)) })
const stagnantTicksLimit = computed<number>({ get: () => (store.simulationParams as any).stagnantTicksLimit, set: v => store.setStagnantTicksLimit?.(Number(v)) })
const plantSpawnRate = computed<number>({ get: () => (store.simulationParams as any).plantSpawnRate, set: v => store.setPlantSpawnRate?.(Number(v)) })
const waterLevel = computed<number>({ get: () => (store.simulationParams as any).waterLevel, set: v => store.setWaterLevel?.(Number(v)) })
function startNext() { store.startSimulation() }
...
function applyPreset(key: PresetKey) {
  ...
  if (typeof p.movementThreshold === 'number') store.setMovementThreshold?.(p.movementThreshold as number)
  if (typeof p.stagnantTicksLimit === 'number') store.setStagnantTicksLimit?.(p.stagnantTicksLimit as number)
  ...
  <div id="controls-panel" class="collapse collapse-arrow bg-base-100 border">
    ...
    <div class="collapse-content">
      <div class="flex flex-wrap gap-2 mb-3" aria-label="Presets">
        <button class="btn btn-xs" :class="lastApplied==='Balanced' ? 'btn-primary' : 'btn-ghost'" @click="applyPreset('Balanced')" aria-label="Apply Balanced preset">Balanced</button>
        <button class="btn btn-xs" :class="lastApplied==='Rapid Growth' ? 'btn-primary' : 'btn-ghost'" @click="applyPreset('Rapid Growth')" aria-label="Apply Rapid Growth preset">Rapid Growth</button>
        <button class="btn btn-xs" :class="lastApplied==='Harsh Climate' ? 'btn-primary' : 'btn-ghost'" @click="applyPreset('Harsh Climate')" aria-label="Apply Harsh Climate preset">Harsh Climate</button>
        <button class="btn btn-xs" :class="lastApplied==='Explorers' ? 'btn-primary' : 'btn-ghost'" @click="applyPreset('Explorers')" aria-label="Apply Explorers preset">Explorers</button>
        <span v-if="lastApplied" class="badge badge-ghost badge-xs" aria-live="polite">Applied: {{ lastApplied }}</span>
      </div>
